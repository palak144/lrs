...
import { IonContent, AlertController, NavController, ModalController, ToastController } from '@ionic/angular';
...
@Component({
  ...
  templateUrl: './uk-home.page.html',
  styleUrls: ['./uk-home.page.scss'],
})
export class UkHomePage implements OnInit {
  ...
  resetForm() {
    ...
    if (this.global.checkIfDataIsValid(this.editleaveInfoObject)) {
      ...
      this.global.trackView('UkHomePage', 'page-uk-home');
    }
  ...
  getPreview() {
    ...
    if ((this.validateLeave() == true) && (!this.leaveObjectFromSummary)) {
      if (this.leaveInfo.leaveType == 'Paternity Leave') {
        if (this.global.checkIfDataIsValid(this.leaveInfo.employee_Comments)) {
          this.validLeaveContinue();
        } else {
          this.global.showDialogs(false, 'As it is paternity leave, comment is mandatory.');
        }
      } else if (this.leaveInfo.leaveType == 'Meeting_Out of Office') {
        if (this.global.checkIfDataIsValid(this.leaveInfo.employee_Comments)) {
          this.validLeaveContinue();
        } else {
          this.global.showDialogs(false, 'As it is meeting/ out of office, comment is mandatory.');
        }
      } else if (this.leaveInfo.leaveType == 'Unpaid Leave UK') {
        if (this.global.checkIfDataIsValid(this.leaveInfo.employee_Comments)) {
          this.validLeaveContinue();
        } else {
          this.global.showDialogs(false, 'As it is unpaid leave, comment is mandatory.');
        }
      } else if (this.leaveInfo.leaveType == 'Shared Parental Leave') {
        if (this.global.checkIfDataIsValid(this.fileToUpload) && this.global.checkIfDataIsValid(this.fileToUpload.name)) {
          this.validLeaveContinue();
        } else {
          this.global.showDialogs(false, 'As it is shared parental leave, attachment is mandatory.');
        }
      } else if (this.leaveInfo.leaveType == 'Maternity Leave') {
        if (this.global.checkIfDataIsValid(this.fileToUpload) && this.global.checkIfDataIsValid(this.fileToUpload.name)) {
          this.validLeaveContinue();
        } else {
      ...
      } else if (this.leaveInfo.leaveType == 'Additional Sick Leave') {
        if (this.global.checkIfDataIsValid(this.fileToUpload) && this.global.checkIfDataIsValid(this.fileToUpload.name)) {
          this.validLeaveContinue();
        } else {
          this.global.showDialogs(false, 'As it is Additional Sick Leave, attachment is mandatory.');
        }
      } else if (this.leaveInfo.leaveType == 'Adoption Leave') {
        if (this.global.checkIfDataIsValid(this.fileToUpload) && this.global.checkIfDataIsValid(this.fileToUpload.name)) {
          this.validLeaveContinue();
        } else {
          this.global.showDialogs(false, 'As it is Adoption Leave, attachment is mandatory.');
        }
      } else {
        this.validLeaveContinue();
      }
      ...
      if (this.leaveInfo.leaveType == 'Paternity Leave') {
        ...
        if (this.global.checkIfDataIsValid(this.leaveInfo.attachedFileLink) ||
          (this.global.checkIfDataIsValid(this.fileToUpload) &&
  ...
  getHomePageNoticeMessage() {
    this.global.getHomePageMessage()
      .subscribe(
        res => {
          this.homeMessage = res.UK;
          this.global.showConsoleLog('homeMessage', this.homeMessage.UK);
        }
      );
  }
  ...
  initializeData(data) {
    ...
    if (this.global.checkIfDataIsValid(this.editleaveInfoObject)) {
      ...
      // tslint:disable-next-line: variable-name
      let startDate_ExpectedFormat, endDate_ExpectedFormat, childAdoptionDate_ExpectedFormat, childBirthDueDate_ExpectedFormat;
      ...
      } else if (this.editleaveInfoObject.LEAVE_TYPE == 'Maternity Leave' || this.editleaveInfoObject.LEAVE_TYPE == 'Paternity Leave') {
  ...
  async successDeleteLeaveCallback(dataFromApi) {
    if (dataFromApi.ErrorCode == 0) {
      ...
      this.global.trackEvent(this.editleaveInfoObject.APPROVAL_STATUS == 'Approved' ?
        'approvedLeaveCancelled' : 'pendingLeaveCancelled', 'UK', this.editleaveInfoObject.id, 0);
      const alert = await this.alertCtrl.create({
        header: this.global.getAlertTitle(),
  ...
  leaveBalance() {
    if (this.global.checkIfDataIsValid(localStorage.UserId) &&
      ...
      if (req) {
        ...
        this.ukhomeservice.calculateLeaves(req)
          .subscribe(
  ...
  checkIfStartDayHalfDay(){
    ...
    return  this.leaveInfo.leaveType == 'Meeting_Out of Office' || this.leaveInfo.leaveType == 'Sick Leave UK' || this.leaveInfo.leaveType == 'Unpaid Leave' || this.leaveInfo.leaveType == 'Additional Sick Leave' || this.leaveInfo.leaveType == "Medical Appointment Leave" || this.leaveInfo.leaveType == "Annual Leave" ?  this.leaveInfo.startHalfDay : '' 
   }
   checkIfEndDayHalfDay(){
    
    return  this.leaveInfo.leaveType == 'Meeting_Out of Office' || this.leaveInfo.leaveType == 'Sick Leave UK' || this.leaveInfo.leaveType == 'Unpaid Leave' || this.leaveInfo.leaveType == 'Additional Sick Leave' || this.leaveInfo.leaveType == "Medical Appointment Leave" || this.leaveInfo.leaveType == "Annual Leave" ?  this.leaveInfo.endHalfDay : '' 
   }
  ...
  applyLeave() {
    ...
    if (this.validateLeave() == true) {
      ...
      formData.append('editLeaveRequest', this.global.checkIfDataIsValid(this.editleaveInfoObject) ?
        'true' : 'false');
      ...
      formData.append('start_day_half_day', this.global.checkIfDataIsValid(this.leaveInfo.startHalfDay) ?
        this.checkIfStartDayHalfDay() : '');
      formData.append('end_day_half_day', this.global.checkIfDataIsValid(this.leaveInfo.endHalfDay) ?
      this.checkIfEndDayHalfDay() : '');
      ...
      this.global.showProgressBar();
      this.ukhomeservice.submitFile(formData)
        .subscribe(
          applyLeaveData => {
  ...
  async successCallbackToApplyLeaves(dataFromApi) {
    ...
    if (dataFromApi.ErrorCode == 0) {
      ...
      if (!this.global.checkIfDataIsValid(this.editleaveInfoObject)) {
        this.global.trackEvent('leaveApplied', 'UK', this.global.getUserId(), 0);
        const alert = await this.alertCtrl.create({
          header: this.global.getAlertTitle(),
          message: dataFromApi.ErrorMessage,
          backdropDismiss: false,
        ...
        this.global.trackEvent('leaveEdited', 'UK', this.global.getUserId(), 0);
        const alert = await this.alertCtrl.create({
          header: this.global.getAlertTitle(),
          message: dataFromApi.ErrorMessage,
          backdropDismiss: false,
  ...
  getCalenderConfigDates(cDate) {
    if (this.global.checkIfDataIsValid(localStorage.UserId) && this.global.checkIfDataIsValid(localStorage.AuthToken)) {
      ...
      if (req) {
        this.global.showProgressBar();
        this.ukhomeservice.getcalenderconfigdates(req)
          .subscribe(
            configDatesData => {
      ...
      for (let j = 0; j < applyStyle.length; j++) {
        ...
        if (day == this.configArray[i].EdgeFormatDate.split('-')[0]) {
          switch (this.options.daysConfig[i].cssClass) {
            ...
            case 'Antenatal_Care_Leave': {
              document.getElementsByClassName('days-btn')[applyStyle[j]].className = 'days-btn Antenatal_Care_Leave marked';
              break;
            }
            case 'Jury_Duty': {
            ...
            case 'Compassionate_Leave': {
              document.getElementsByClassName('days-btn')[applyStyle[j]].className = 'days-btn Compassionate_Leave marked';
              break;
            }
            case 'Shared_Parental_Leave': {
              document.getElementsByClassName('days-btn')[applyStyle[j]].className = 'days-btn Shared_Parental_Leave marked';
              break;
            }
            case 'Meeting_Out_of_Office': {
              document.getElementsByClassName('days-btn')[applyStyle[j]].className = 'days-btn Meeting_Out_of_Office marked';
              break;
            }
            case 'Sick_Leave_UK': {
              document.getElementsByClassName('days-btn')[applyStyle[j]].className = 'days-btn Sick_Leave_UK marked';
              break;
            }
        ...
        if (this.global.checkIfDataIsValid(calendarDate) && this.isSelectedDateIsClickable == true) {
          leaveObj = {
            ...
            DURATION: (this.global.checkIfDataIsValid(this.tempDateFromHolidayList) &&
              (this.global.checkIfDataIsValid(this.tempDateFromHolidayList.DURATION))) ?
              this.tempDateFromHolidayList.DURATION : '',
            id: (this.global.checkIfDataIsValid(this.tempDateFromHolidayList) &&
              (this.global.checkIfDataIsValid(this.tempDateFromHolidayList.id))) ?
              this.tempDateFromHolidayList.id : '',
  ...
  selectedLeaveType(leaveType) {
    ...
    if ((leaveType == 'Annual Leave' || leaveType == 'Additional Sick Leave' ||
      leaveType == 'Unpaid Leave UK' || leaveType == 'Jury Duty' || leaveType == 'Shared Parental Leave' ||
      leaveType == 'Antenatal Care Leave' || leaveType == 'Sick Leave UK' ||
      leaveType == 'Compassionate Leave' || leaveType == 'Medical Appointment Leave' ||
      leaveType == 'Meeting_Out of Office' || leaveType == 'Work From Home') &&
    ...
    if ((leaveType == 'Adoption Leave' || leaveType == 'Additional Adoption Leave') &&
      this.global.checkIfDataIsValid(this.leaveInfo.startDate) && this.global.checkIfDataIsValid(this.leaveInfo.endDate) &&
    ...
    if ((leaveType == 'Paternity Leave' || leaveType == 'Maternity Leave') &&
      this.global.checkIfDataIsValid(this.leaveInfo.startDate) && this.global.checkIfDataIsValid(this.leaveInfo.endDate) &&
      this.global.checkIfDataIsValid(this.leaveInfo.startHalfDay)) {
      this.calculateDuration('');
    }
    ...
    if (this.global.checkIfDataIsValid(this.leaveInfo.startDate) && this.global.checkIfDataIsValid(this.leaveInfo.endDate)) {
      if (this.validateLeave() == true) {
        ...
        if (this.compareDate(new Date(this.leaveInfo.startDate), new Date(this.leaveInfo.endDate)) == 1) {
          ...
          if (req) {
            this.global.showProgressBar();
            this.ukhomeservice.calculateDuration(req)
              .subscribe(
                leavedurationdata => {
  ...
  clearLeaveForm() {
    ...
    this.zone.run(() => this.global.router.navigate(['/uk-home'], { skipLocationChange: true }));
  }
      ...
      if (this.global.checkIfDataIsValid(data.data)) {
        if (from == 'startDate') {
          ...
          if ((this.leaveInfo.leaveType == 'Annual Leave' ||
            this.leaveInfo.leaveType == 'Additional Sick Leave' ||
            this.leaveInfo.leaveType == 'Unpaid Leave UK' ||
            this.leaveInfo.leaveType == 'Jury Duty' ||
            this.leaveInfo.leaveType == 'Shared Parental Leave' ||
            this.leaveInfo.leaveType == 'Antenatal Care Leave' ||
            this.leaveInfo.leaveType == 'Sick Leave UK' ||
            this.leaveInfo.leaveType == 'Compassionate Leave' ||
            this.leaveInfo.leaveType == 'Medical Appointment Leave' ||
            this.leaveInfo.leaveType == 'Work From Home' ||
            this.leaveInfo.leaveType == 'Meeting_Out of Office') &&
            this.global.checkIfDataIsValid(this.leaveInfo.endDate)) {
            this.calculateDuration('');
          }

          // Apdoption Date Mandatory 
          if ((this.leaveInfo.leaveType == 'Adoption Leave' ||
            this.leaveInfo.leaveType == 'Additional Adoption Leave') &&
            this.global.checkIfDataIsValid(this.leaveInfo.endDate) &&
            this.global.checkIfDataIsValid(this.leaveInfo.startHalfDay)) {
            this.calculateDuration('');
          }
          ...
          // Child BirthDate Mandatory 
          if ((this.leaveInfo.leaveType == 'Paternity Leave' ||
            this.leaveInfo.leaveType == 'Maternity Leave') &&
            this.global.checkIfDataIsValid(this.leaveInfo.endDate) &&
            this.global.checkIfDataIsValid(this.leaveInfo.startHalfDay)) {
            this.calculateDuration('');
          }
          ...
          if ((this.leaveInfo.leaveType == 'Maternity Leave' ||
            this.leaveInfo.leaveType == 'Paternity Leave') &&
            this.global.checkIfDataIsValid(this.leaveInfo.endDate) &&
